...
//    uint64_t initialCreate = ofTime().getAsNanoseconds();
	mesh = geo;
	int level = 0;
	...
	else {} // need to load face vertices here
	// recursively buid octree
	level++;
    subdivide(mesh, root, numLevels, level);
//    uint64_t finalCreate = ofTime().getAsNanoseconds();
//    cout << finalCreate - initialCreate << " ms" << endl;
}
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
    ...
    vector<Box> meshBoxList;
    subDivideBox8(node.box, meshBoxList); // creates 8 child boxes into meshBoxList
    vector<int> pointsReturned;
    
    for (int i = 0; i < meshBoxList.size(); i++) {
        pointsReturned.clear();
        getMeshPointsInBox(mesh, node.points, meshBoxList.at(i), pointsReturned); // use mesh points instead of faces (Project 3)
        if (pointsReturned.size() >= 1) {
            TreeNode childNode;
            childNode.box = meshBoxList.at(i);
            childNode.points = pointsReturned;
            node.children.push_back(childNode);
            if (pointsReturned.size() > 1) { // if not a leaf node
